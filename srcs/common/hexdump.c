...
ssize_t hexdump(const uint8_t *addr, size_t n, size_t offset)
{
	...
	while (n) {
		...
		tmp += pointer_16_bytes((uint8_t *)tmp, offset);
		*(tmp++) = ' ';
		*(tmp++) = ' ';
		tmp += data_16_bytes_color((uint8_t *)tmp, ptr, line_size);
		*(tmp++) = ' ';
		*(tmp++) = ' ';
		tmp += ascii_16_bytes((uint8_t *)tmp, ptr, line_size);
		offset += 16;
		ptr += 16;
		(void)write(STDOUT_FILENO, buffer, tmp - buffer);
		(void)write(1, "\n", 1);
	}
...
size_t data_16_bytes_color(uint8_t *buffer, const uint8_t *addr, size_t n)
{
	...
	while (n) {
		if (char_color[*ptr] == CYAN) {
			(void)memcpy(buffer, "\033[0;36m", 7);
			buffer += 7;
			...
			if (!*ptr) {
				(void)memcpy(buffer, "\033[2;37m", 7);
				buffer += 7;
				while (n && !*ptr) {
					*(uint32_t *)(buffer) =
						hex_char_pair[*ptr++];
...
size_t data_16_bytes(const uint8_t *dst, const uint8_t *src, size_t n)
{
	...
	(void)memcpy((void *)dst, __padding, (16 - n) * 3);
	return (47);
}

size_t pointer_16_bytes(uint8_t *dst, uintptr_t p)
{
